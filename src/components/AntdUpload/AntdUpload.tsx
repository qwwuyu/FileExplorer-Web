...
import type { RcFile, ShowUploadListInterface, UploadChangeParam, UploadFile } from 'antd/lib/upload/interface';
import { file2Obj, getFileItem, removeFileItem, updateFileList } from 'antd/lib/upload/utils';
import UploadList from 'antd/lib/upload/UploadList';
import useStyle from 'antd/lib/upload/style';
import { UploadProps } from 'antd';

export const LIST_IGNORE = `__LIST_IGNORE_${Date.now()}__`;

const InternalUpload = (props: UploadProps<any>, ref: React.ForwardedRef<unknown>) => {
  const {
    fileList,
    defaultFileList,
    onRemove,
  ...
  const upload1 = React.useRef<RcUpload>(null);
  const upload2 = React.useRef<RcUpload>(null);
  ...
  const handleRemove = (file: UploadFile) => {
    ...
    Promise.resolve(typeof onRemove === 'function' ? onRemove(file) : onRemove).then((ret) => {
      ...
      if (removedFileList) {
        ...
        upload1.current?.abort(currentFile as RcFile);
        upload2.current?.abort(currentFile as RcFile);
  ...
  const { showRemoveIcon, showPreviewIcon, showDownloadIcon, removeIcon, previewIcon, downloadIcon } =
  ...
  if (true || type === 'drag') {
      ...
      <>
        {wrapSSR(
          <span className={classNames(`${prefixCls}-wrapper`, rtlCls, className, hashId)}>
            <div className={dragCls} onDrop={onFileDrop} onDragOver={onFileDrop} onDragLeave={onFileDrop} style={style}>
              <RcUpload {...rcUploadProps} ref={upload1} className={`${prefixCls}-btn`}>
    ...
    const exportUploadList = () =>
      wrapSSR(
        <span className={classNames(`${prefixCls}-wrapper`, rtlCls, className, hashId)}>{renderUploadList()}</span>
      );

    const UploadAble = (children, className) => {
      return (
        <RcUpload {...rcUploadProps} ref={upload2} className={className} openFileDialogOnClick={false}>
          {children}
        </RcUpload>
      );
    };

    return { exportUpload, exportUploadList, UploadAble };
...
const AntdUpload = InternalUpload;
